...
class LSPooling(torch.nn.Module):
    ...
    def forward(
        ...
        # add self-loop 
        # replace add remaining self loops with directly adding self loops
        edge_index_with_selfloop, edge_attr_with_selfloop = add_self_loops(edge_index, edge_attr)

        # select returns dense select out
        scores = self.lssmp.forward(
            x=x,
            edge_index=edge_index_with_selfloop,
            edge_attr=edge_attr_with_selfloop,
        )
        ...
        # local set collapse
        x = self.lscmp(x, scores.unsqueeze(-1), edge_index_with_selfloop)
